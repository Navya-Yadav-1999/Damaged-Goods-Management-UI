...
import { addInspections, updateInspections, getInspectionById } from "./InspectionService";
...
const InspectionForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = id ? await updateInspections(id, inspectionData) : await addInspections(inspectionData);
  ...
  return (
    <Container maxWidth="lg">
      <Box component="form" onSubmit={handleSubmit} sx={{ padding: 3, backgroundColor: "#f9f9f9", borderRadius: 2 }}>
        ...
        <Grid container spacing={2}>
          {["inspectorName", "inspectionDate", "location", "findings", "severity", "actionTaken", "photos"].map((field) => (
            <Grid item xs={12} sm={6} key={field}>
              <TextField
                label={field.replace(/([A-Z])/g, " $1").trim()}
                name={field}
                value={inspections[field]}
                onChange={handleChange}
                fullWidth
                variant="outlined"
              />
